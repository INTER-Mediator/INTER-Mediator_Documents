...
window.onload = function() {
    ...
    if (typeof sitemap !== 'undefined') {
        ...
        setTitle(path);
        // ブレッドクラム要素の追加
        var targetNode = getNodeOfBreadcrumb(); // ブレッドクラムを追加する要素
        if (targetNode !== false) {
            var currentItem = getItemFromSitemap(path); // 現在の項目を取得
            addBreadcrumbItem(currentItem, targetNode); // 現在のブレッドクラム項目を追加
            addParentItemToBreadCrumb(currentItem.parent); // 親ブレッドクラム項目の追加
        }
    }
...
function search() {
    ...
    if (word !== '') {
        ...
        xhr.onload = function(e) {
            if (xhr.readyState === 4) {
                if (xhr.status === 200) {
                    pageContents = JSON.parse(xhr.responseText);
                    pageContentsLowerCase = JSON.parse(xhr.responseText.toLowerCase());
                    /* init lunr */
...
function setTitle(path) {
    ...
    var currentItem = getItemFromSitemap(path);
    if (document.getElementById('pageTitle') !== null) {
        document.getElementById('pageTitle').textContent = currentItem.title;
    }
    document.title = currentItem.title + suffix;
}

// ブレッドクラム項目の追加
function addBreadcrumbItem(item, targetNode) {
    "use strict";
...
// 親項目ブレッドクラムの追加
function addParentItemToBreadCrumb(parentPath) {
    "use strict";
    var targetNode = getNodeOfBreadcrumb();
    if (targetNode === false) {
        return;
    } else {
        var item = getItemFromSitemap(parentPath);
        if (item !== false) {
            addBreadcrumbItem(item, targetNode);
            addParentItemToBreadCrumb(item.parent);
        }
    }
}
